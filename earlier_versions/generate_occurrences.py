"""
Cette version a été créée dans le but de créer les fichhiers .txt uniquement,
à partir de tous les albums. Elle permet de créer deux fichiers d'occurences :
un avec les mots classés par ordre d'apparition dans le dernier album (mots_decroissant.txt),
et un par ordre alphabétique (mots_alphab.txt)
"""
...
if __name__ == "__main__":
    ...
    # Ordre décroissant
    with open("earlier_versions/outputs/mots_decroissant.txt", "w") as f:
        for i in range(NB_ALBUMS + 1):
            f.write(f"###### Album {i} ######\n{write_dict(count_dict[i], 'd')}\n")
    
    # Ordre alphabétique
    with open("earlier_versions/outputs/mots_alphab.txt", "w") as f:
        for i in range(NB_ALBUMS + 1):
